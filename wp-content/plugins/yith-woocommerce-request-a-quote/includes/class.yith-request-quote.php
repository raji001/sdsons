...
if ( ! class_exists( 'YITH_Request_Quote' ) ) {
	...
	class YITH_Request_Quote {
		...
		public function ajax_add_item( $posted ) {
			...
			if ( ! $is_valid ) {
				$errors[] = __( 'Please select required fields', 'yith-woocommerce-request-a-quote' );
			} else {
		...
		public function send_message() {
			...
			if ( ! isset( $_POST['rqa_name'] ) ) {
				...
				$rqa_phone = sanitize_text_field( wp_unslash( $_POST['rqa_phone'] ) );
				if ( empty( $rqa_phone ) ) {
					$errors[] = '<p>' . __( 'Please enter a mobile no', 'yith-woocommerce-request-a-quote' ) . '</p>';
				}
