...
class OptimizerMain
{
    ...
    {
        if ( OptimizerUrl::isUrlAMP( $content ) ) {
            self::add_optimization_status_headers( 0, 'Do not optimize AMP pages' );
            return $content;
        }
        ...
        } else if ($this->cacheStructure->getCacheStatus()) {
        ...
        if (!$this->cacheStructure->isFromCache()) {
            ...
            if(!empty($twoptimizeStyles->two_async_css_arr) || !empty($twoptimizeScripts->two_js_list) || !empty($this->critical_fonts_arr) || !empty($twoptimizeScripts->two_js_list_excluded)){
                ...
                $content = OptimizerUtils::inject_in_html($content, $two_worker_data_to_inject_critical_status, array('<head', 'after_tag'));
                ...
                $this->cacheStructure->addToTagsToAdd($two_worker_data_to_inject_critical_status, array('<head', 'after_tag'));
